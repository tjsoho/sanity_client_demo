"use strict";
var path = require("node:path"), node_util = require("node:util"), chalk = require("chalk"), logSymbols = require("log-symbols"), semver = require("semver"), telemetry = require("@sanity/telemetry"), rimraf$1 = require("rimraf"), previewServer = require("./previewServer.js"), runtime = require("./runtime.js"), checkRequiredDependencies = require("./checkRequiredDependencies.js"), timing = require("./timing.js"), fs = require("node:fs"), resolveFrom = require("resolve-from"), vite = require("vite");
...
async function compareStudioDependencyVersions(autoUpdatesImports, workDir, fetchFn = globalThis.fetch) {
  const manifest = checkRequiredDependencies.readPackageJson(path__default.default.join(workDir, "package.json")), dependencies = { ...manifest.dependencies, ...manifest.devDependencies }, failedDependencies = [], filteredAutoUpdatesImports = Object.entries(autoUpdatesImports).filter(
    ([pkg]) => !pkg.endsWith("/")
  );
  for (const [pkg, value] of filteredAutoUpdatesImports) {
    ...
    const dependency = dependencies[pkg], manifestPath = resolveFrom__default.default.silent(workDir, path__default.default.join(pkg, "package.json")), installed = semver__default.default.coerce(
      manifestPath ? checkRequiredDependencies.readPackageJson(manifestPath).version : dependency
    );
    if (!installed)
...
async function buildSanityStudio(args, context, overrides) {
  ...
  const { didInstall, installedSanityVersion } = await checkRequiredDependencies.checkRequiredDependencies(context);
  if (didInstall)
    return { didCompile: !1 };
