...
async function getMonorepoAliases(monorepoPath) {
  const { default: aliases } = await Promise.resolve().then(function() {
    return require("./dev-aliases.js");
  }).then(function(n) {
    return n.devAliases;
  });
  return Object.fromEntries(
    Object.entries(aliases).map(([pkgName, pkgPath]) => [pkgName, path__default.default.resolve(monorepoPath, path__default.default.join("packages", pkgPath))])
  );
}
