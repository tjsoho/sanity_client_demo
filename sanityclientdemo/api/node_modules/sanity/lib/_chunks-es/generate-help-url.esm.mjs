...
import { Button as Button$1, Dialog as Dialog$1, Flex, Box, ErrorBoundary as ErrorBoundary$1, MenuButton as MenuButton$1, MenuGroup as MenuGroup$1, Hotkeys as Hotkeys$1, Text, Stack, Badge, MenuItem as MenuItem$1, Popover as Popover$1, Tab as Tab$1, Tooltip as Tooltip$1, TooltipDelayGroupProvider as TooltipDelayGroupProvider$1 } from "@sanity/ui";
import { forwardRef, useCallback, useContext, useMemo, isValidElement, createElement } from "react";
...
function ErrorBoundary({ onCatch, ...rest }) {
  const source = useContext(SourceContext), handleCatch = useCallback(
    ({ error: caughtError, info: caughtInfo }) => {
      source?.onUncaughtError?.(caughtError, caughtInfo), onCatch?.({ error: caughtError, info: caughtInfo });
    },
    [source, onCatch]
  );
