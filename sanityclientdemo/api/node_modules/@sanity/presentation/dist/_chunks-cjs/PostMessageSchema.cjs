"use strict";
var ui = require("@sanity/ui"), react = require("react"), index = require("./index.cjs"), sanity = require("sanity"), server = require("react-dom/server"), jsxRuntime = require("react/jsx-runtime"), styledComponents = require("styled-components");
...
const SchemaIcon = function({ schemaType, theme: themeContext }) {
  const { theme, scheme, tone } = themeContext, sheet = new styledComponents.ServerStyleSheet();
  return schemaType.icon ? /* @__PURE__ */ jsxRuntime.jsx(styledComponents.StyleSheetManager, { sheet: sheet.instance, children: /* @__PURE__ */ jsxRuntime.jsx(ui.ThemeProvider, { theme, scheme, tone, children: react.createElement(schemaType.icon) }) }) : null;
...
function extractSchema(workspace, theme) {
  ...
  function extractIcon(schemaType) {
    if (schemaType.icon)
      return server.renderToString(react.createElement(SchemaIcon, { schemaType, theme }));
  }
...
function PostMessageSchema(props) {
  const { comlink, perspective } = props, workspace = sanity.useWorkspace(), theme = ui.useRootTheme();
  react.useEffect(() => {
  ...
  const client = sanity.useClient({ apiVersion: index.API_VERSION });
  return react.useEffect(() => comlink.on("visual-editing/schema-union-types", async (data) => {
