'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var sanity = require('sanity');
var constants = require('./constants.cjs');
var icons = require('@sanity/icons');

const debugUrlSecretsType = sanity.defineType({
  type: "document",
  icon: icons.LockIcon,
  name: constants.schemaType,
  title: "@sanity/preview-url-secret",
  readOnly: true,
  fields: [
    {
      type: "string",
      name: "secret",
      title: "Secret"
    },
    {
      type: "string",
      name: "source",
      title: "Source Tool"
    },
    {
      type: "string",
      name: "studioUrl",
      title: "Studio URL"
    },
    {
      type: "string",
      name: "userId",
      title: "Sanity User ID"
    }
  ],
  preview: {
    select: {
      source: "source",
      studioUrl: "studioUrl",
      updatedAt: "_updatedAt"
    },
    prepare(data) {
      const url = data.studioUrl ? new URL(data.studioUrl, location.origin) : void 0;
      const updatedAt = new Date(data.updatedAt).getTime();
      const expiresAt = new Date(updatedAt + 1e3 * constants.SECRET_TTL);
      const expired = expiresAt < /* @__PURE__ */ new Date();
      const icon = expired ? icons.CloseCircleIcon : icons.CheckmarkCircleIcon;
      return {
        title: url ? `${url.host}${url.pathname}` : data.source,
        subtitle: expired ? "Expired" : `Expires in ${Math.round((expiresAt.getTime() - Date.now()) / (1e3 * 60))} minutes`,
        media: icon
      };
    }
  }
});

const debugSecrets = sanity.definePlugin(() => {
  return {
    name: "sanity-plugin-debug-secrets",
    schema: {
      types: [debugUrlSecretsType]
    },
    document: {
      actions: (prev, context) => {
        if (context.schemaType !== constants.schemaType) {
          return prev;
        }
        return prev.filter(({ action }) => action === "delete");
      },
      inspectors: (prev, context) => {
        if (context.documentType !== constants.schemaType) {
          return prev;
        }
        return [];
      },
      unstable_fieldActions: (prev, context) => {
        if (context.schemaType.name !== constants.schemaType) {
          return prev;
        }
        return [];
      }
    }
  };
});

exports.debugSecrets = debugSecrets;
//# sourceMappingURL=sanity-plugin-debug-secrets.cjs.map
